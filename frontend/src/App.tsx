...
export default function App() {
  ...
  useEffect(() => {
    fetch(`${BACKEND_URL}/properties`)
      .then((res) => res.json())
      .then((data) => Array.isArray(data) && data.length > 0 && setActiveProperties(data))
  ...
  const sendChatMessage = async (textMessage: string) => {
    ...
    const userMessage: ChatMessage = { id: Date.now().toString(), sender: "user", text: textMessage }
    ...
    try {
      ...
      if (data.recommendations?.length > 0) setActiveProperties(data.recommendations)
    } catch {
      setMessages((prev) => [
        ...prev,
        {
  ...
  const handleSetWorkplaceClick = () => {
    const placeName = prompt("Enter your workplace address or city (e.g., 'BGC Taguig'):")
    if (placeName?.trim()) sendChatMessage(`My workplace is at ${placeName.trim()}`)
  }
  ...
  return (
    <div className="flex h-screen w-screen flex-col overflow-hidden bg-slate-50 font-sans md:flex-row">
      ...
      <div className={`flex h-full w-full flex-col border-r bg-white shadow-sm md:w-[420px] lg:w-[480px] ${activeTab === "chat" ? "flex" : "hidden md:flex"}`}>
        <Header
      ...
      <div className={`h-full flex-1 flex-col p-2 sm:p-4 ${activeTab === "map" ? "flex" : "hidden md:flex"}`}>
